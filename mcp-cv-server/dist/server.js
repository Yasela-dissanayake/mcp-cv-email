...
function answerCvQuestion(resume, q) {
    ...
    if (/(what|which).*(role|title).*last (position|job|role)/.test(question)) {
        if (!last)
            return "I couldn't find any work entries.";
...
app.get("/", (_req, res) => {
    res
        .type("text/plain")
        .send("MCP CV Server: /health, POST /ask, POST /send-email, HTTP MCP at /mcp");
});
...
// Tool: send_email
mcpServer.tool("send_email", { to: z.string().email(), subject: z.string(), body: z.string() }, async ({ to, subject, body }) => {
    const result = await sendEmail({ to, subject, body });
